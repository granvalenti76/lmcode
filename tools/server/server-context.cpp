...
struct server_slot {
    ...
    // Prompt cache stats for this request
    int32_t n_prompt_tokens_cached = 0;  // Number of tokens reused from cache in current request
...
struct server_metrics {
    ...
    // prompt cache stats
    uint64_t n_prompt_cache_hits   = 0;
    uint64_t n_prompt_cache_misses = 0;
    uint64_t n_prompt_cache_savings_tokens = 0;
    ...
    void on_prompt_cache_hit(int32_t n_cached_tokens) {
        n_prompt_cache_hits++;
        n_prompt_cache_savings_tokens += n_cached_tokens;
    }

    void on_prompt_cache_miss() {
        n_prompt_cache_misses++;
    }

    float get_prompt_cache_hit_rate() const {
        uint64_t total = n_prompt_cache_hits + n_prompt_cache_misses;
        return total > 0 ? (float)n_prompt_cache_hits / total : 0.0f;
    }
...
//
// server_metrics_writer
//

struct server_metrics_writer {
    std::string file_path;
    std::ofstream file;
    int64_t t_start = 0;

    // cumulative stats for hit rate calculation
    ...
    bool init(const std::string & path) {
        ...
        if (!file.is_open()) {
            SRV_ERR("failed to open metrics file: %s\n", file_path.c_str());
            return false;
        }

        t_start = ggml_time_us();
        SRV_INF("metrics logging enabled: %s\n", file_path.c_str());
        return true;
    }

    void log(const server_slot & slot, const server_metrics & metrics,
             int queue_depth, int slots_busy, int slots_total,
             int32_t n_prompt_tokens_cached) {
        ...
        // token counts
        const int32_t input_tokens = slot.n_prompt_tokens_processed;
        const int32_t output_tokens = slot.n_decoded;
        const int32_t total_tokens = input_tokens + output_tokens;

        // timing
        const double time_to_first_token_ms = timings.prompt_ms;
        const double time_total_ms = timings.prompt_ms + timings.predicted_ms;
        const double time_per_token_ms = output_tokens > 0 ? timings.predicted_ms / output_tokens : 0.0;

        // throughput
        const double tokens_per_second_total = time_total_ms > 0 ? (total_tokens / time_total_ms * 1000.0) : 0.0;
        const double input_tokens_per_second = timings.prompt_ms > 0 ? (input_tokens / timings.prompt_ms * 1000.0) : 0.0;
        const double output_tokens_per_second = timings.predicted_ms > 0 ? (output_tokens / timings.predicted_ms * 1000.0) : 0.0;

        // cache stats
        const bool cache_hit = n_prompt_tokens_cached > 0;
        const float cache_hit_rate_cumulative = metrics.get_prompt_cache_hit_rate();

        // KV cache - get used tokens from memory
        auto * memory = llama_get_memory(slot.ctx);
        const int32_t n_kv_cache_tokens = memory ? llama_memory_seq_pos_max(memory, slot.id) + 1 : 0;

        // KV cache usage percent
        const int32_t n_ctx = slot.n_ctx;
        const float kv_cache_usage_percent = n_ctx > 0 ? (float)n_kv_cache_tokens / n_ctx * 100.0f : 0.0f;

        // stop reason
        std::string stop_reason;
        switch (slot.stop) {
            case STOP_TYPE_EOS:   stop_reason = "eos"; break;
            case STOP_TYPE_WORD:  stop_reason = "stop_word"; break;
            case STOP_TYPE_LIMIT: stop_reason = "length"; break;
            default:              stop_reason = "unknown"; break;
        }

        // generation ratio
        const float generation_length_ratio = task && task->params.n_predict > 0
            ? (float)output_tokens / task->params.n_predict
            : 0.0f;

        // build JSON line
        char buffer[2048];
        snprintf(buffer, sizeof(buffer),
            "{\"timestamp\":%" PRId64 ",\"request_id\":\"req_%lu\","
            "\"input_tokens\":%d,\"output_tokens\":%d,\"total_tokens\":%d,"
            "\"tokens_per_second_total\":%.1f,\"input_tokens_per_second\":%.1f,\"output_tokens_per_second\":%.1f,"
            "\"time_to_first_token_ms\":%.1f,\"time_per_token_ms\":%.2f,\"time_total_ms\":%.1f,"
            "\"prompt_cache_hit\":%s,\"prompt_cache_hit_rate_cumulative\":%.3f,"
            "\"prompt_cache_tokens_reused\":%d,\"prompt_cache_savings_tokens_total\":%" PRIu64 ","
            "\"kv_cache_usage_percent\":%.1f,\"kv_cache_tokens\":%d,"
            "\"queue_depth\":%d,\"slots_busy\":%d,\"slots_total\":%d,"
            "\"stop_reason\":\"%s\",\"generation_length_ratio\":%.2f}\n",
            timestamp,
            (unsigned long)requests_total,
            input_tokens, output_tokens, total_tokens,
            tokens_per_second_total, input_tokens_per_second, output_tokens_per_second,
            time_to_first_token_ms, time_per_token_ms, time_total_ms,
            cache_hit ? "true" : "false", cache_hit_rate_cumulative,
            n_prompt_tokens_cached, metrics.n_prompt_cache_savings_tokens,
            kv_cache_usage_percent, n_kv_cache_tokens,
            queue_depth, slots_busy, slots_total,
            stop_reason.c_str(), generation_length_ratio);
    ...
    ~server_metrics_writer() {
        if (file.is_open()) {
            file.close();
            SRV_INF("metrics logging disabled: %s\n", file_path.c_str());
        }
    }
};
...
private:
    ...
    server_metrics_writer metrics_writer;
    ...
    bool init() {
        ...
        metrics.init();
        metrics_writer.init(params_base.metrics_file);
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT || slot.state == SLOT_STATE_STARTED) {
                    ...
                    if (slot.state == SLOT_STATE_STARTED) {
                        ...
                        if (input_tokens.empty()) {
                            ...
                            // log metrics if enabled (empty response)
                            const int queue_depth = 0;
                            int slots_busy = 0;
                            for (const auto & s : slots) { if (s.is_processing()) slots_busy++; }
                            metrics_writer.log(slot, metrics, queue_depth, slots_busy, slots.size(), 0);
                        ...
                        if (!slot.can_split()) {
                            ...
                            if (slot.task->params.cache_prompt) {
                                ...
                                if (slot.alora_invocation_start > 0) {
                                    SLT_DBG(slot, "only caching to alora invocation start (n_past = %d, alora_invocation_start = %d)", n_past, slot.alora_invocation_start);
                                    n_past = std::min(n_past, slot.alora_invocation_start - 1);
                                }

                                // save cached tokens count for metrics (before any cache shifting)
                                // this is the number of tokens that were a cache HIT (common prefix)
                                const int32_t n_cache_hit_initial = n_past;
                                slot.n_prompt_tokens_cached = n_cache_hit_initial;

                                // track prompt cache hit/miss (based on initial common prefix)
                                if (n_cache_hit_initial > 0) {
                                    metrics.on_prompt_cache_hit(n_cache_hit_initial);
                                    SLT_DBG(slot, "prompt cache HIT: reusing %d tokens", n_cache_hit_initial);
                                } else {
                                    metrics.on_prompt_cache_miss();
                                    SLT_DBG(slot, "prompt cache MISS: no tokens to reuse", "");
                                }
        ...
        for (int32_t i = 0; i < batch.n_tokens; i = i_next) {
            ...
            for (auto & slot : slots) {
                ...
                if (!process_token(result, slot)) {
                    ...
                    int slots_busy = 0;
                    for (const auto & s : slots) { if (s.is_processing()) slots_busy++; }
                    metrics_writer.log(slot, metrics, queue_depth, slots_busy, slots.size(), slot.n_prompt_tokens_cached);
