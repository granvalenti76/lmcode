<script lang="ts">
	...
	// Estrae il nome del modello per esteso (es: "Qwen3-32B" da "ggml-org/Qwen3-32B-GGUF:Q8_0")
	let shortModelName = $derived.by(() => {
		const name = activeModelId ?? conversationModel;
		if (name) {
			// Rimuove il prefisso user/org (es: "ggml-org/")
			const withoutOrg = name.split('/').pop() ?? name;
			// Rimuove il suffisso GGUF e quantizzazione (es: "-GGUF:Q8_0")
			const withoutSuffix = withoutOrg.replace(/-GGUF(:[A-Z0-9_]+)?$/i, '');
		...
		const options = modelOptions();
		if (options.length > 0) {
			const firstModel = options[0].model;
			const withoutOrg = firstModel.split('/').pop() ?? firstModel;
			return withoutOrg.replace(/-GGUF(:[A-Z0-9_]+)?$/i, '');
		}
...
<div class="flex w-full items-center gap-2 {className}" style="container-type: inline-size">
	<!-- Sinistra: Nome modello per esteso -->
	<div class="flex items-center gap-2 min-w-0">
		<span class="text-sm font-medium truncate" title={activeModelId ?? ''}>
			{shortModelName}
		</span>
	</div>

	<!-- Destra: Upload File + Reasoning Budget + Bottone invio/stop -->
	<div class="flex items-center gap-2 ml-auto">
		<!-- Bottone Upload File (Desktop: Dropdown, Mobile: Sheet) -->
