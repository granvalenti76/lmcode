...
class ChatStore {
	...
	private getApiOptions(): Record<string, unknown> {
		...
		// thinking_budget_tokens: 0 means unlimited (not disabled)
		if (currentConfig.thinking_budget_tokens !== undefined)
			apiOptions.thinking_budget_tokens = Number(currentConfig.thinking_budget_tokens);

		// enable_thinking: pass to server via chat_template_kwargs
		if (currentConfig.enable_thinking !== undefined)
			apiOptions.enable_thinking = currentConfig.enable_thinking;
